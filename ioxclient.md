...
To display current CA-certificate used by the platform, use the below command.
```
~$ioxclient platform certificate get
Currently active profile :  router829
Command Name:  plt-cert-get
SSL/TLS Certificate that is being used by the device:
------------------------
-----BEGIN CERTIFICATE-----
MIICrDCCAZQCCQCGweam5oMlczANBgkqhkiG9w0BAQsFADAYMRYwFAYJKoZIhvcN
...

-----END CERTIFICATE-----
...
NAME:
   set - Import SSL/TLS certificate on the platform

USAGE:
   command set <certificate> [key]

DESCRIPTION:

To Import the certificate on the platform:
    ioxclient platform certificate set <certificate>

To Import the certificate and private key on the platform:
    ioxclient platform certificate set <certificate> <key>
...
~$ioxclient plt cert s ~/ioxsce.cert.pem ~/ioxsce.key.pem
Currently active profile :  my7
Command Name:  plt-cert-set
Importing ssl certificate will restart application hosting server with new certificate. Do you want to proceed?[y/N]:y
Moving forward with execution of the command
Response from the server: Imported ssl certificate. IOx app hosting framework will be restarted in few seconds with imported ssl certificate
...
COMMANDS:
   ...
   descriptor, desc		Generate a package descriptor yaml file
...
Usage: ioxclient docker package <image_name or docker rootfs file path> <project_dir>
image_name  -> is a valid docker image name.
project_dir -> should be a directory location that has the required IOx files.
...
Build an image using Docker CLIs:
...
Now build IOx package using ioxclient and specify the newly created Docker image:
...
* Note: One can increase the size of the rootfs by using the "--headroom" option to specify additional disk space (in MB) that needs to be added 
...
### Generate a specific schema version package descriptor yaml file

With docker workflow support, you can use any existing docker image (public/private) to generate a package descriptor yaml file.

```
$ ioxclient  docker descriptor
Insufficient Args.

NAME:
   descriptor - Generate package descriptor yaml file from existing docker image.

USAGE:
   command descriptor [schema version] <docker_image> <project_dir>

DESCRIPTION:

Usage: ioxclient docker descriptor [command options] <image_name> <project_dir>
schema version -> schema version number. If schema version is not provided, default descriptor file package.yaml is generated.
image_name  -> is a valid docker image name.
project_dir -> should be a directory location that generated package yaml file will be saved.

Example: docker descriptor alpine:latest .
image_name is alpine:latest
project_dir is "." signifying current directory
Default package descriptor package.yaml is generated at current directory
...
Example: docker descriptor 2.14 alpine:latest .
schema version is 2.14
image_name is alpine:latest
project_dir is "." signifying current directory
Package descriptor file 2.14_package_version.yaml is generated at current directory

OPTIONS:
   --schema-version, -v 		Use this option to specify schema version.

```
An example to generate specified schema version package descriptor yaml file

ioxclient docker desc -v 2.11 schema_2.15:latest pkg_proj_dir/
Currently active profile :  ir829_18
Command Name:  docker-descriptor
Timestamp at DockerDescriptor start: 1611354229588
Package descriptor yaml file will be generated:  2.11_package_version.yaml
Setting the descriptor schema version to 2.11
Validating generated descriptor file.
Validating descriptor file /tmp/desc543638904 with package schema definitions
Parsing descriptor file..
Found schema version  2.11
Loading schema file for version  2.11
Validating package descriptor file..
File /tmp/desc543638904 is invalid under schema version 2.11. Errors below:

app - Additional property child is not allowed
Property will be removed under schema version  2.11
...
Package descriptor file is generated at:  pkg_proj_dir/2.11_package_version.yaml

```
An example to generate unspecified schema version package descriptor yaml file

ioxclient docker desc schema_2.15:latest pkg_proj_dir/Currently active profile :  ir829_18
Command Name:  docker-descriptor
Timestamp at DockerDescriptor start: 1611354416455
...
Validating generated descriptor file.
Validating descriptor file /tmp/desc508904664 with package schema definitions
Parsing descriptor file..
Found schema version  2.14
Loading schema file for version  2.14
Validating package descriptor file..
File /tmp/desc508904664 is invalid under schema version 2.14. Errors below:
